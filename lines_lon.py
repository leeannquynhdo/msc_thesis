...
def train_until_convergence(model, train_set, val_set, epochs, patience):
    ...
    print("Total running time for lines lon unet:", total_time)
    
    return model, train_loss_list, val_loss_list


if should_train:
    trained_model, train_losses, val_losses = train_until_convergence(lon_model, training_generator, validation_generator, epochs=500, patience=500)
...
trained_model = lon_model.to(device)
...
for i in range(len(test_images)):
    ...
    plt.savefig(f'line_unet/line_models/test_output/lon_test_result_{i}.png', dpi=500, bbox_inches='tight')
    plt.close()
...
plt.legend()
plt.show()
plt.savefig('line_unet/line_models/line_lon_train_val_loss.png', dpi=500, bbox_inches='tight')
plt.close()
