...
path_to_trained_model = 'unet_conv_models/trained_unet_model_convnext_mse.pth'
path_to_train_loss = 'unet_conv_models/unet_convnext_all_train_losses_mse.txt'
path_to_val_loss = 'unet_conv_models/unet_convnext_all_val_losses_mse.txt'
...
def train_until_convergence(model, train_set, val_set, epochs, patience):
    ...
    print("Total running time for unet with convnext pretrained weights using MSELoss", total_time)
...
for i in range(len(re_test_images)):
    ...
    plt.savefig(f'unet_conv_models/unet_conv_test_result_mse_{i}.png', dpi=500, bbox_inches='tight')
    plt.close()
    # plt.show()
...
plt.legend()
plt.savefig(f'unet_conv_models/unet_conv_train_val_loss_mse.png', dpi=500, bbox_inches='tight')
plt.close()
...
plt.title('U-Net ConvNeXt MSE ROC curve')
plt.plot(fpr, tpr, label=f'AUC score = {auc}')
plt.legend()
plt.savefig('unet_conv_models/roc_curve_mse.png', dpi=500, bbox_inches='tight')
plt.close()

print("U-Net ConvNeXt BCE")

accuracy = torch.eq(predictions, labels).sum().item() / len(predictions)

# with open(f'unet_conv_models/unet_conv_accuracy_mse.txt', 'w') as f:
#     f.write("%s\n" % accuracy)
