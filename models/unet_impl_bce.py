...
path_to_model = 'unet_models/trained_unet_model_bce.pth'
path_to_train_loss = 'unet_models/unet_all_train_losses_bce.txt'
path_to_val_loss = 'unet_models/unet_all_val_losses_bce.txt'
...
def train_until_convergence(model, train_set, val_set, epochs, patience):
    ...
    print("Total running time for unet without pretrained weights using BCELoss", total_time)
...
for i in range(len(re_test_images)):
    ...
    plt.savefig(f'unet_models/unet_test_result_bce_{i}.png', dpi=500, bbox_inches='tight')
    plt.close()
    # plt.show()
...
plt.legend()
plt.savefig(f'unet_models/unet_train_val_loss_bce.png', dpi=500, bbox_inches='tight')
plt.close()
...
# with open(f'unet_models/unet_accuracy_bce.txt', 'w') as f:
#     f.write("%s\n" % accuracy)
