...
path_to_trained_model = 'lon_unet_models/trained_lon_unet_model_mse.pth'
path_to_train_loss = 'lon_unet_models/lon_unet_train_losses_mse.txt'
path_to_val_loss = 'lon_unet_models/lon_unet_val_losses_mse.txt'
...
def train_until_convergence(model, train_set, val_set, epochs, patience):
    ...
    for epoch in range(epochs):
        ...
        if (epoch%50==0): #save every model every 50th iter
            torch.save(model.state_dict(), f'lon_unet_models/lon_unet_model_mse_epoch_{epoch+1}.pth' )
...
for i in range(len(re_test_images)):
    ...
    plt.savefig(f'lon_unet_models/lon_unet_mse_test_result_{i}.png', dpi=500, bbox_inches='tight')
    plt.close()
    plt.show()
...
plt.legend()
plt.savefig(f'lon_unet_models/lon_unet_train_val_loss_mse.png', dpi=500, bbox_inches='tight')
plt.close()
...
plt.legend()
plt.savefig('lon_unet_models/roc_curve_mse.png', dpi=500, bbox_inches='tight')
plt.close()
